...
# Setup
~~~
```{r setup-figure-4b, echo = FALSE, include = FALSE, eval = TRUE}
...
```{r extract-relevant-data-for-figure-4b}
# Figure name
FigNam <- "Figure 4b"
...
Here, we present the serum chemistry analysis of a total of *N*=`r unique(TotN)` well-nourished (WN) or malnourished (MN) BALB/c mice infected by sand fly bite (SF) or not (Ctrl) (Appendix table `r TabNum + 1`). A total of `r length(TisNam)` targets were measured (`r stringi::stri_replace_last(paste(TisNam, collapse = ", "), fixed = ",", " &")`) per mouse.

```{r table-126a-sample-size-information, include = TRUE, eval = TRUE}
...
```{r table-126b-sample-size-information, include = TRUE, eval = TRUE}
# Sample size information by cell type
sapply(TisNam, function(zz) {
  ...
  gt(rownames_to_stub = TRUE) %>%
    ...
    tab_header(
      title = md("**Appendix Table 126**"),
      subtitle = md("**Summary information (continued)**")
    ) %>%
    tab_style(
      style = cell_text(align = "center"),
      locations = cells_column_labels()
...
The assessment of the untransformed data distribution for each group was conducted by Shapiro-Wilks test and QQ-plot after splitting the data by both predictors. Note that all groups consisted of <30 data points (Appendix table `r TabNum + 1`), which made groups too small to assess data distribution reliably by Shapiro-Wilks test. We executed the test anyway as an indicator of gross departure of data normality. Thus, we performed the analyses by Shapiro-Wilks test (Appendix table `r TabNum + 2`) and QQ-plots (Fig.S1a-1). We found occasional departure of normality, which could not all be remedied by data transformation.

```{r table-127-shapiro-wilks, include = TRUE, eval = TRUE}
...
lapply(TisNam, function(qq) {
  ...
  gt() %>%
    tab_header(
      title = md("**Appendix Table 127**"),
      subtitle = md("**Univariate Shapiro-Wilks test results**")
    ) %>%
    tab_style(
      style = cell_text(align = "center"),
      locations = cells_column_labels()
...
```{r qq-plot-figure-4b, include = TRUE, eval = TRUE, fig.align = "center", fig.height = 9, out.width = "95%", fig.cap = "Fig.4b-1: QQ-plots of serum chemistry data: A) alanine aminotransferase (ALT), B) albumin (ALB), C) alkaline phosphatase (ALP), D) amylase (AMY), E) Globulin (GLOB), F) Glucose (GLU), G) total protein (TP) and H) urea nitrogen (BUN)"}
ggarrange(SubDat[["alanine aminotransferase (ALT)"]][["QQ plot: by groups"]],
          SubDat[["albumin (ALB)"]][["QQ plot: by groups"]],
          SubDat[["alkaline phosphatase (ALP)"]][["QQ plot: by groups"]],
          SubDat[["amylase (AMY)"]][["QQ plot: by groups"]],
          SubDat[["Globulin (GLOB)"]][["QQ plot: by groups"]],
          SubDat[["Glucose (GLU)"]][["QQ plot: by groups"]],
          SubDat[["total protein (TP)"]][["QQ plot: by groups"]],
          SubDat[["urea nitrogen (BUN)"]][["QQ plot: by groups"]], 
          labels = LETTERS[1:8],
          ncol = 2, 
          nrow = 4)
```
...
The assessment of homogeneity of variance was conducted by Levene's test for the dataset. The analysis output showed that assumption of homogeneity between groups held all targets accept for alkaline phosphatase (Appendix table `r TabNum + 3`).

```{r table-128-homogeneity-of-variance, include = TRUE, eval = TRUE}
...
Conversely, we detected outliers for all targets except for alkaline phosphatase (Appendix table `r TabNum + 4`). Only one of these outliers was classed as extreme (Globulin (GLOB)).

```{r table-129-outlier-test, include = TRUE, eval = TRUE}
# Data outliers
lapply(TisNam, function(qq) {
    SubDat[[qq]][["Outliers"]]
  }) %>%
...
Based on the assumption analysis, we applied appropriate two-way tests for each target (Appendix table `r TabNum + 5`).

```{r table-130a-applied-tests, include = TRUE, eval = TRUE}
# Applied tests
cbind(
...
```{r table-130b-applied-tests, include = TRUE, eval = TRUE}
# Applied tests
cbind(
  lapply(SubDat, function(aa) {
    aa[["Assumption_Test_Outcome"]]["2-way_ANOVA_Test_applied", "Outcome"]
  ...
  gt(rownames_to_stub = TRUE) %>%
    ...
    tab_header(
      title = md("**Appendix Table 130**"),
      subtitle = md("**Applied tests (continued)**")
    ) %>%
    tab_style(
      style = cell_text(align = "center"),
      locations = cells_column_labels()
...
Since both predictors had only two levels each, the pairwise comparison reflected the observations of the one-way analyses above (Appendix table `r TabNum + 8`).

```{r table-133-pairwise-comparison, include = TRUE, eval = TRUE}
# Pairwise comparison
...
Considering the small group sizes (Appendix tables `r TabNum + 1`), we wanted to ensure that the study design was not significantly underpowered. Thus, we performed a retrospective sample size and power analysis on the data by target.

```{r subsetting-the-power-calculation-data-for-figure-4b}
# Subset data for figure 4b
SubDat <- PwrCal2w %>%
  .[names(.) %like% FigNam] %>%
  purrr::set_names(TisNam)
...
```{r table-135-sample-size, include = TRUE, eval = TRUE}
...
```{r table-136-power-calculation, include = TRUE, eval = TRUE}
...
```{r update-table-number-figure-4b}
TabNum <- TabNum + 11
```