...
Dat2w <- RawDat[names(RawDat) %like% "mix2"]
...
Dat2wSpt <- lapply(c(1:length(Dat2wSpt)), function(zz) {
  ...
  # Replace the values
  Dat2wSpt[[zz]][Dat2wSpt[[zz]]$Time_point %in% "Week_0", "Weight_g"] <- WgtW0
...
Res2wMix <- lapply(Dat2wSpt, function(bb) {
...
# ASSUMPTION TESTING ###########################################################
  ...
  for (aa in 1:100) {
    ...
    NormUV <- ExpDat %>% 
      ...
      facet_grid(Time_point ~ Diet, labeller = "label_both")
