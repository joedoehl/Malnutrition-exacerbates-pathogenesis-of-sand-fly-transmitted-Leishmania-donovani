...
xxx <- lapply(DatCntSpt[names(DatCntSpt) %like% "2e-f" |
                         names(DatCntSpt) %like% "3d" |
...
ResCnt <- mapply(function(aa, zz) {
  ...
  lapply(ZroInf, function(cc) {
      ...
      lapply(RegMods, function(dd) {
        ...
        E2 <- residuals(Mod, type = "pearson")
        N  <- nrow(aa)
        
        if (dd == "negative_binomial") {
  
          # For negative binomial regression
          p  <- length(coef(Mod)) + 1  # '+1' is for variance parameter in NB
        ...
        DispTest3 <- c(DispTest3, "var/mean" = DispTest3["var"] / DispTest3["mean"])
...
ModSelAIC <- lapply(AIC, function(gg) {
...
ResAssump <- lapply(RegModAIC, function(aa) {
  ...
  NormRes <- shapiro_test(residuals(aa[[1]][["Regression model"]])) %>% # Shapiro-Wilks test of model residuals
  ...
  plotRes <- ggqqplot(residuals(aa[[1]][["Regression model"]]), ggtheme = theme_bw())
...
# pastecs::stat.desc(DatCntSpt$`Figure 2c Cnt`$Counts)
