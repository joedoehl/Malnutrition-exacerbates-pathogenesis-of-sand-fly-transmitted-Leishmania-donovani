...
# Setup
~~~
```{r setup-figure-2d, echo = FALSE, include = FALSE, eval = TRUE}
...
```{r extract-relevant-data-for-figure-2d}
# Figure name
FigNam <- "Figure 2d"
...
We analyzed a total of *N*=`r TotN` BALB/c mice (`r paste(names(SampleN), SampleN, sep = "=", collapse = ", ")`). These were the same mice as analyzed in figure 2b for parasite dissemination events. Here, we quantified parasite burden per isolated spleen. For the data analysis, we tested several Poisson and negative binomial-type regression models. Based on the Akaike information criterion (AIC), we selected a `r RegModAIC %>% .[names(.) %like% FigNam] %>% .[[1]] %>% names(.) %>% gsub("*- ", "", .)` regression model for the data analysis. The model fit of the data was moderate, producing no statistically significant departure from 1 for its dispersion ratio (`r paste(names(DispRat), DispRat, sep = ": ", collapse = ", ")`), but producing only a pseudo-R^2^ of `r pR2[,2]` (`r pR2[,1]`). The model output showed that "Diet" was the only statistically significant predictors, with "Route" and the interaction term producing no statistically significant result (Appendix table `r TabNum + 1`).

```{r table-76-regression-analysis-of-counts, include = TRUE, eval = TRUE}
# Model output
RegModAIC %>%
  .[names(.) %like% FigNam] %>%
...
```{r table-78-pairwise-comparison-letter, include = TRUE, eval = TRUE}
RegModAIC%>%
  .[names(.) %like% FigNam] %>%
  .[[1]] %>% # pulls out the data from the list
  .[[1]] %>% # picks the only model output left
...
```{r update-table-number-figure-2d}
TabNum <- TabNum + 3
```
