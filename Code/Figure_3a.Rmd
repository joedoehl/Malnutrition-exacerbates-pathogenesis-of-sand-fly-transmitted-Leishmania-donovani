...
We would need to analyze the data with a three-way approach to account for the three predictors, "Time_point" was the within-subject factor, while "Diet" and "Route" were the between-subject factors in the analysis with "Weight_g" being the dependent outcome variable.

For a three-way mixed ANOVA, we had to assess the data for compliance with assumptions:
!!!!!!!!!!!!!!!!!!!!!!!! covariance and sphericity assumption test !!!!!!!!!!!!!!!
-   Data normality
...
The assessment of the untransformed data distribution for each group was conducted by Shpiro-Wilks test and QQ-plot after splitting the data by all three predictors. Note that all groups consisted of *N*=`r GrpN` individuals, which made groups too small to assess data distribution reliably by Shapiro-Wilks test. In spite of this, we performed the analyses by Shapiro-Wilks test (Table `r TabNum + 1`) and QQ-pots (Fig.3a-1) and found deviations from normality.
...
```{r qq-plot-figure-3a, include = TRUE, eval = TRUE, fig.align = "center", fig.height = 9, out.width = "95%", fig.cap = "Fig.3a-1: QQ-plots of myeloid cell counts split into groups by predictor variables"}
...
The assessment of homogeneity of variance was conducted by Levene's test for the dataset split by the within-subject factor ("Time_point"). The analysis output showed that assumption of homogeneity between groups held for each week (Table `r TabNum + 2`).
...
It can be difficult to determine outliers in small datasets reliably as the analysis is dependent on the interquartile range of the data per group. We attempted it anyway and found a total of `r nrow(SubDat[[SubFigNam]][["Outliers"]])` hypothetical outliers of which `r SubDat[[SubFigNam]][["Outliers"]] %>% .[.[["is.extreme"]],] %>% nrow(.)` were classed as extreme (Table `r TabNum + 3`).
...
Based on the assumption analysis, we decided to apply a `r SubDat[[SubFigNam]][["Assumption_Test_Outcome"]]["3-way_analysis_applied",]` to the dataset to determine the effects of "Diet", infection "Route" and time post infection ("Time_point") on mouse weight over time (Table `r TabNum + 4`). The test output showed that all three individual predictors were a statistically significant, while only the two two-way interactions with "Diet" were statistically significant. There was no statistically significant three-way interaction.
...
For the analysis of the simple simple main effect for each respective between-subject factor, we performed `r SubDat[[SubFigNam]][["Assumption_Test_Outcome"]]["1-way_ANOVA_Test_applied",]` with individual between-subject factor of the data split by the other two predictors. The results showed that "Diet" caused statistically significant differences with the exception of "Week_0", which prior to the assignment of special diets (Table `r TabNum + 6`). "Route only showed occasionally statistical significant difference; most commonly between "Week_6" and "Week_18", which was only associated with the malnourished group (Table `r TabNum + 7`).
...
For the pairwise comparison, we applied a `r SubDat[[SubFigNam]][["Assumption_Test_Outcome"]]["Pairwise_comparison_applied",]`. Since the "Diet" predictor only had two factor levels, the output showed the same result as the `r SubDat[[SubFigNam]][["Assumption_Test_Outcome"]]["1-way_ANOVA_Test_applied",]` above. For the "Route" predictor, the pairwise comparison presented a more detailed view at where statistically significant differences occurred (Table `r TabNum + 8`). As for the `r SubDat[[SubFigNam]][["Assumption_Test_Outcome"]]["1-way_ANOVA_Test_applied",]` above, all statistical significant differences were observed between "Week_6" and "Week_18" and were restricted to the malnourished groups (Table `r TabNum + 9`). The main differences between the malnourished groups resided primarily with the needle inoculated group from "Week_6" to "Week_14", who had more weight gain than either the malnourished control or sand fly infected groups. From "Week_15" onword, the average mouse weight for the malnourished control group approached that or the needle group and statistical differences were now only observed compared to the malnourished sand fly infected group, that never seemed to gain weight post infection.
...
In conclusion, that "Diet" was the most potent predictor for mouse weight gain over time. Interestingly, between the well-nourished groups, we never observed statistically significant weight gains over time, suggesting that in that state, and conversely to the malnourished mouse groups, mouse weight was not affected by infection status or "Route" in well-nourished mice. These data support the hypothesis that the nutritional state of an individual can directly impact their weight before and during *Leishmania donovani* infection.
